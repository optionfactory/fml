...
- [REF] `ful-badge`, the chip of a `multiple` `ful-select`, is a neutral value chip: selected background, border and text colors instead of the accent fill it borrowed from `--ful-active-bg`, which read as a primary action button and buried a field under a wall of accent color. It also tints on hover, as the removable control it always was
